...
app.get('/exercises2', (req, res) => {
	...
	const results = musicData.filter(data => 10 <= data.name.length 
		&& data.name.length <= 25 );
...
app.get('/exercises3', (req, res) => {
	...
	*/
	...
	const totalAlbumSales = musicData
			.reduce((prevState,data)=>{
				console.log(data);
				return prevState + data.sales
			},0);

	console.log(totalAlbumSales);
	
	res.json(totalAlbumSales);
	})
	...
	*/
	...
	const totalAlbumSales = musicData
		.filter(album => album.artist.length + album.name.length < 25)
		.reduce((accumulator, value) =>{
		return accumulator + value.sales;
	},0);
	...
	*/
	app.get('/exercises6', (req, res) => {
		...
		.reduce((prevState,{favoriteIceCreams}) =>[...prevState, ...favoriteIceCreams],[])
		.reduce((prevState,flavor) => ({ ...prevState, [flavor]: (prevState[flavor] || 0) + 1 }),{})
	
	console.log(iceCreamTotals);
		
	res.json(iceCreamTotals);
